...
//		routing.joinCheck();
	...
	public void join(String uri) throws Failed, Invalid {
		setPred(null);
		NodeInfo info = getNodeInfo();
		NodeInfo succ = findSuccessor(URI.create(uri),info.id);
		setSucc(succ);
		stabilize();
		/*
	...
	public EventOutput listenForBindings(int id, String key) {
		...
		System.out.println("listforBindings("+key+")");
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 */
		int id = NodeKey(key);
		NodeInfo info = findPredecessor(id);
		info = getSucc(info);
		//System.out.println("Found [" + key + "] bindings at "+ info.id + "");
		EventSource eventSource = client.listenForBindings(info,this.info.id,key);
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		NodeInfo info = findPredecessor(id);
		info = getSucc(info);
		client.listenOff(info,this.info.id,key);
